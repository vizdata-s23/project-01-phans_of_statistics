...
Our first question asks if contestants from the `host_country` in each competition tend to perform better than foreign contestants. We were interested in this question because hosting a competition at home is often seen as an advantage, especially in sports such as basketball and soccer where having a supportive crowd and familiar environment is beneficial. Consequently, we wanted to see if there was a similar pattern for singing competitions like Eurovision, and if there was, to what extent it correlates with performance.
